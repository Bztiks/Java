...
20. 使用类的时候是单独定义，例如`class name`，里面再定义不同的类型的方法名，并且这个定义不在初始的{}之内，而是在{}之外，类可以看成是创建Java对象的模板
```java
class Photos{
    String A(String a)
    {
...
21. 每个类都有<font color="red">构造方法</font>，没有显式的为类定义构造方法，Java编译器将会为该类提供一个默认构造方法

22. （对象构造）多个方法有相同的名字，不同的参数，便产生了<font color="red">重载</font>，通过不同的方法给出的参数类型与特定方法调用所使用的值的类型进行匹配来挑选出相应的方法，<font color="red">不能有两个名字相同，参数类型也相同，返回值类型不同的方法</font>

23. （包）在导入所需要的包的时候要注意冲突性

24. import语句不仅仅可以导入类，还可以导入静态方法和静态域，特定的方法和域
